...
class GoBackTest {
    ...
    @Test
    fun goToBack(){
        Espresso.onView(ViewMatchers.withId(R.id.rvBestFlimandSeries)).perform(
            RecyclerViewActions.actionOnItemAtPosition<BestPopularFilmViewHolder>(0,
                ViewActions.click()
            )
        )
        Espresso.onView(ViewMatchers.withId(R.id.tvYear))
